...
export function getCurrentChapter(pathname: string): Chapter | undefined {
  return chapters.find((chapter) => pathname.includes(chapter.slug));
}

export function getAdjacentChapters(pathname: string): {
  previous: Chapter | null;
  next: Chapter | null;
} {
  const currentIndex = chapters.findIndex((chapter) =>
    pathname.includes(chapter.slug)
  );
